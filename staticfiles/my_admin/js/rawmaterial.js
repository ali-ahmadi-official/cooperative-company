document.addEventListener("DOMContentLoaded", function () {
    ...
    const commodityCategorySelect = document.getElementById("id_commodity_category");
    const commoditySelect = document.getElementById("id_commodity");

    function loadCategoriesByTernal(ternal) {
        if (!ternal) return;

        fetch(`/get-categories-by-ternal/?ternal=${ternal}`)
            .then(response => response.json())
            .then(data => {
    ...
    function loadCommodityCategoriesByCategory(categoryId) {
        if (!categoryId) return;

        fetch(`/get-commodity-categories-by-category/?category_id=${categoryId}`)
            .then(response => response.json())
            .then(data => {
                commodityCategorySelect.innerHTML = '<option value="">---------</option>';
                data.forEach(item => {
                    const option = document.createElement("option");
                    option.value = item.id;
                    option.textContent = item.name;
                    commodityCategorySelect.appendChild(option);
                });
            });
    }

    function loadCommoditiesByCategory(commodityCategoryId) {
        if (!commodityCategoryId) return;

        const params = new URLSearchParams();
        params.append("commodity_category_id", commodityCategoryId);
    ...
    if (ternalSelect) {
        ternalSelect.addEventListener("change", function () {
            loadCategoriesByTernal(this.value);
        });
    }

    if (categorySelect) {
        categorySelect.addEventListener("change", function () {
            loadCommodityCategoriesByCategory(this.value);
        });
    }

    if (commodityCategorySelect) {
        commodityCategorySelect.addEventListener("change", function () {
            loadCommoditiesByCategory(this.value);
        });
    }
});
